...
label day_3:
    ...
    e "[name] приходит в магазин и осматривает витрины магазина."
    ...
    if first_root_day_3:
        ...
        e "[name] прикупил себе парочку вещей и вернулся домой."
    ...
    if second_root_day_3:
        e "[name] ничего себе не взял и вернулся домой."


        scene room
        
        m 'Привет, солнце. Как то ты странно выглядишь. Рассказывай всё, что произошло.'
        g "Мам... совсем недавно я нашёл очень много денег, они просто валялись в кустах. Я боялся тебе об этом сказать... п..п..рости меня."
        m "Ах ты мелкий негодник... Ты же знаешь, что у нас и так мало денег в семье."
        g "Ты не мой сын, мой сын не стал бы мне врать."

        jump end_2
    
    if third_root_day_3:
        g "Чтож мелочиться, скуплю себе всё, что тут найдёт"
        e "Глупый [name] носился по магазинам и скупал всё, что только видел.{w=3}Он совсем не следил за временем и магазин начал закрываться.{w=3}Охранник подашёл к мальчику."
        ...
        hide s normal
        hide gr mone
        e "Полиция приехала за мальчиком, чтобы отвезти его домой. Полицейский был смущён таким большим количеством покупок у мальчика и тем, что он совсем один."
